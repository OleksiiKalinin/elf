...
const QuestionsScreen: React.FC = () => {
          ...
          buttons: [
            {
              ...
              onPress: () => replace({ stack: 'MenuStack', screen: 'QuestionEditorScreen', params: {id: id} })
            },
            {
              children: 'Usuń listę',
              icon: <SvgIcon icon='crossBig' />,
              closeAction: 'props-null',
              onPress: () => {deleteList(), replace({ stack: 'MenuStack', screen: 'QuestionsListScreen', params: undefined })}
            },
          ],
        }
        return null;
  ...
  const deleteList = () =>{
    if(index !== null){
      const newList = cloneDeep(userQuestions);
      setUserQuestions(newList.splice(index, 1));
    };
  };

  const handleSubView = () => {
    if(id){
      router.push({ stack: 'MenuStack', screen: 'QuestionsScreen', params: {id: id, subView: 'options'}});
    };
  };
